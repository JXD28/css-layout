...
- 浏览器将html文件解析成一个对象(节点)树 ->DOM
- 浏览器解析到CSS时候会获取并解析css文件->css对象模型(树形结构),每一个选择器都会匹配一个DOM节点
- 渲染树:每隔节点表示要渲染到屏幕上的信息,与DOM节点不完全一样,为元素不会在DOM中出现,但在渲染树中.还包括滚动块或者视口->构建完渲染树,节点都应该知道自己是什么颜色等
- 浏览器将数据转换成屏幕显示,由浏览器自己掌握(DOM和CSSOM都是标准化)
- 布局:浏览器会遍历渲染树,确认每个对象显示在页面的位置
- 绘制,合成与呈现:浏览器根据渲染数的信息,把元素准确地呈现到屏幕上.复杂元素->GPU
- 优化渲染性能:页面发生变化需要重绘,最好在16ms内完成绘制,16ms对应的屏幕刷新频率为60Hz,最不影响性能的就是在合成阶段完成的操作,比如改变文本颜色